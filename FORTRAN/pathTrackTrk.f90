! gfortran pathTrackTrk.f90 ./libocpiddae1.a ./libfgssqp.a ./libspline.a ./libroadhelper.a
! ./a.out
MODULE RoadRefSpeed
   IMPLICIT NONE
...
PROGRAM pathTrackTrk
   ...
   P(1, 3) = 1.4D0     ! upper bound for eta_v relaxation
   ...
   fileName = 'velDataTrk.dat' ! File with [s, v]
   ...
   fileName = "roadDataTrk.dat" ! File with (x,y,z,slopel,slopeq,width) for reference line
   ...
   DO
      ...
      IF (currStateFSM == 10) THEN ! Exit Parking (XP)
         ...
         IF (((XL(0, 1) - ROAD_S(0)) >= 20.0D0) .AND. (maxSpeed >= 8.5D0)) THEN
            ...
            UL(-1:1, 1) = -0.2D0 - (-0.1D0*(1.0D0 - transPhaseVal))
            UU(-1:1, 1) = 0.2D0 - (0.1D0*(1.0D0 - transPhaseVal))
            UL(-1:1, 2) = -4.0D0 - (-1.0D0*(1.0D0 - transPhaseVal))
            UU(-1:1, 2) = 3.5D0 - (1.5D0*(1.0D0 - transPhaseVal))
            ! State limits (constraints)
            USER(2) = -0.4D0 - (-0.2D0*(1.0D0 - transPhaseVal)) ! kappa_min
            USER(3) = 0.4D0 - (0.2D0*(1.0D0 - transPhaseVal)) ! kappa_max
            USER(4) = 0.0D0 ! v_min
         ...
         ELSE
            ...
            UU(-1:1, 2) = 3.5D0
            ! State limits (constraints)
            USER(2) = -0.4D0 ! kappa_min
            USER(3) = 0.4D0 ! kappa_max
            USER(4) = 0.0D0 ! v_min
            USER(5) = 2.7D0 ! v_max
         END IF
      ...
      ELSE IF (currStateFSM == 20) THEN ! Path Following (PF)
         ...
         IF ((ROAD_S(ROAD_NPOINTS) - XL(0, 1)) <= 100.0D0) THEN
            ...
            UL(-1:1, 1) = -0.1D0 - (0.1D0*(1.0D0 - transPhaseVal))
            UU(-1:1, 1) = 0.1D0 - (-0.1D0*(1.0D0 - transPhaseVal))
            UL(-1:1, 2) = -3.0D0 - (1.0D0*(1.0D0 - transPhaseVal))
            UU(-1:1, 2) = 2.0D0 - (-1.5D0*(1.0D0 - transPhaseVal))
            ! State limits (constraints)
            USER(2) = -0.2D0 - (0.2D0*(1.0D0 - transPhaseVal)) ! kappa_min
            USER(3) = 0.2D0 - (-0.2D0*(1.0D0 - transPhaseVal)) ! kappa_max
            USER(4) = 0.0D0 ! v_min
         ...
         ELSE IF ((maxSpeed < 13.5D0) .OR. ((O_inLn > 0) .AND. (XL(0, 5) < 13.5D0))) THEN
            ...
            UU(-1:1, 2) = 2.0D0 - (-1.5D0*(1.0D0 - transPhaseVal))
            ! State limits (constraints)
            USER(2) = -0.2D0 ! kappa_min
            USER(3) = 0.2D0 ! kappa_max
            USER(4) = 0.0D0 ! v_min
...
SUBROUTINE NLCSTR(T, X, U, P, G, IUSER, USER)
   ...
   G(3) = (X(2))**2 - 0.4D0
   G(4) = (X(2) + 1.6D0*(X(3)))**2 - 0.4D0
   G(5) = (X(2) + 3.2D0*(X(3)))**2 - 0.4D0
