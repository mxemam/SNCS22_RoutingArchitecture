! gfortran optVelCar.f90 ./libocpiddae1.a ./libfgssqp.a ./libspline.a ./libroadhelper.a
! ./a.out
MODULE RoadSpline
   IMPLICIT NONE
...
PROGRAM optVelCar
   ...
   G(0, 1) = -0.8D0    ! kappa_min
   G(1, 1) = 0.8D0     ! kappa_max
   G(0, 2) = -15.0D0   ! v_min
   G(1, 2) = 0.0D0     ! v_max: v_max - v <= 0
   G(0, 3) = -5.0D0    ! minimum lateral acceleration
   G(1, 3) = 5.0D0     ! maximum lateral acceleration

   ! Open file for writing all OCP data
   fileName = 'optVelCar.dat'
   OPEN (11, FILE=fileName, STATUS='UNKNOWN')
   fileName = 'roadDataCar.dat'
   OPEN (22, FILE=fileName, STATUS='UNKNOWN')
   fileName = 'velDataCar.dat'
